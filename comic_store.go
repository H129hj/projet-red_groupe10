...
func ComicBookStore(c *Character, progress *ScenarioProgress) {
	...
	typeWriter("🏪 Vous entrez dans lantre du Comic Book Guy...", 15*time.Millisecond)
	typeWriter("📚 Des milliers de BD, figurines et objets geek sentassent partout.", 15*time.Millisecond)
	typeWriter("", 15*time.Millisecond)
	...
	case "lisa":
		...
		typeWriter("👨‍💻 Comic Book Guy : Enfin quelquun avec un cerveau ! Contrairement à ton père...", 15*time.Millisecond)
	case "maggie":
	...
	case 1:
		...
		typeWriter("👨‍💻 Comic Book Guy : Il parl'ait dun \"Grand Concours du Donut Cosmique\" au parc dattractions.", 15*time.Millisecond)
	...
	case 2:
		typeWriter("👨‍💻 Comic Book Guy : QUOI ?! Laudace ! L'impudence !", 15*time.Millisecond)
		...
		if fightChoice == 1 {
			...
			comicGuyMonster := InitMonster("Comic Book Guy (Critique Ultime)", 350, 30)
			ScenarioCombat(c, &comicGuyMonster, progress, comicGuyPattern, "comic")
			return
		} else {
			typeWriter("👨‍💻 Comic Book Guy : Hmph ! Bon... ton père parl'ait du parc dattractions.", 15*time.Millisecond)
			typeWriter("👨‍💻 Comic Book Guy : Quelque chose à propos dun concours de donuts...", 15*time.Millisecond)
			typeWriter("", 15*time.Millisecond)
			typeWriter("🔍 INDICE OBTENU : Homer est au parc dattractions pour un concours !", 15*time.Millisecond)
			progress.HasClue3 = true
			progress.ComicCompleted = true
			progress.Stage = 4
		}
	...
	case 3:
		typeWriter("👨‍💻 Comic Book Guy : *rougit* Mes connaissances encyclopédiques ! Enfin quelquun qui apprécie !'", 15*time.Millisecond)
		typeWriter("👨‍💻 Comic Book Guy : Ton père, Homer Simpson, est venu avec un ticket mystérieux...", 15*time.Millisecond)
		typeWriter("👨‍💻 Comic Book Guy : Il sagit du légendaire \"Grand Concours du Donut Cosmique\"...", 15*time.Millisecond)
		typeWriter("👨‍💻 Comic Book Guy : Organisé par des forces... extraterrestres... au parc dattractions !", 15*time.Millisecond)
		typeWriter("👨‍💻 Comic Book Guy : Le gagnant obtient des donuts magiques pour léternité !", 15*time.Millisecond)
		typeWriter("", 15*time.Millisecond)
		typeWriter("🔍 INDICE OBTENU : Concours extraterrestre de donuts au parc dattractions !", 15*time.Millisecond)
		progress.HasClue3 = true
		progress.ComicCompleted = true
		progress.Stage = 4
...
func characterTurnComicGuy(c *Character, m *Monster, t int, progress *ScenarioProgress) {
	...
	if c.PV <= 0 {
		...
		case 3:
			...
			typeWriter("👨‍💻 Comic Book Guy : Fuyez ! Votre père est au parc dattractions !", 15*time.Millisecond)
			progress.HasClue3 = true
			progress.ComicCompleted = true
			progress.Stage = 4
			ScenarioMenu(c, progress)
